...
### Rule 11: Data Provenance Labeling
  ...
  *   ✅ 三种标签全部显式标注：`[销售确认]`、`[网络搜索]`、`[AI推断]`，无隐含默认
...
```bash
# 1. 保存 markdown（路径按 Section 6 命名规范）
# 假设文件存为 {output_dir}/EP_Customer_Opportunity.md

# 2. 渲染 PDF（必须）
python -m shared {md_file} -f pdf -o {output_dir}/EP_Customer_Opportunity.pdf --strict

# 3. 渲染 HTML（必须，中间产物存档）
python -m shared {md_file} -f html -o {output_dir}/EP_Customer_Opportunity.html --strict

# 4. 渲染 DOCX（按需，销售明确要求时）
python -m shared {md_file} -f docx -o {output_dir}/EP_Customer_Opportunity.docx --strict
```
